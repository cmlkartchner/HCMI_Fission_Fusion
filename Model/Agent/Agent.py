...
class Agent:
    def __init__(self, id, hex, movement_speed=1):
        self.id = id
            ...
            if reading.trails:
                for trailHex in reading.trails.values():
                    if self.hex.computeDistance(trailHex):
                        ...
                        intent = total_pheromone_strength / \
                            (self.hex.computeDistance(trailHex))**2
                ...
                for loc in reading.sites.keys():
                    if not self.memory.contains(loc):
                        ...
                        if self.hex.computeDistance(site.hex):
                            ...
                            intent = site.quality / \
                                (self.hex.computeDistance(site.hex))**2
                    ...
                    for agent in agents:
                        if self.hex.computeDistance(agent.hex):
                            ...
                            if isinstance(self.state, YearningState):
                                ...
                                intent = self.getAttractionCoefficient(
                                    agent)/(self.hex.computeDistance(agent.hex))**2
